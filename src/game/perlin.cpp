...
double Perlin::octavePerlin(double x, double y, double z, int octaves,
        double persistence) {
    double total = 0;
    double frequency = 1;
    double amplitude = 1;
    ...
    for (int i = 0; i < octaves; i++) {
        total += perlin(x * frequency, y * frequency, z * frequency)
                * amplitude;
        maxValue += amplitude;
...
double Perlin::perlin(double x, double y, double z) {
    ...
    double xf = x - floor(x); // We also fade the location to smooth the result.
    double yf = y - floor(y);
    double zf = z - floor(z);
    double u = fade(xf);
    double v = fade(yf);
