...
void GL20Renderer::destroyRenderDistanceDependent() {
...
void GL20Renderer::initRenderDistanceDependent() {
...
void GL20Renderer::resize(int width, int height) {
...
void GL20Renderer::makePerspective() {
...
void GL20Renderer::makeOrthogonal() {
	double normalRatio = DEFAULT_WINDOWED_RES[0] / (double) DEFAULT_WINDOWED_RES[1];
	double currentRatio = width / (double) height;
	glMatrixMode(GL_PROJECTION);
	glLoadIdentity();
...
void GL20Renderer::calcDrawArea() {
	double normalRatio = DEFAULT_WINDOWED_RES[0] / (double) DEFAULT_WINDOWED_RES[1];
	double currentRatio = width / (double) height;
	if (currentRatio > normalRatio) {
...
void GL20Renderer::setMenu(bool menuActive) {
...
void GL20Renderer::setDebug(bool debugActive) {
	this->debugActive = debugActive;
}

bool GL20Renderer::isDebug() {
	return debugActive;
}
...
void GL20Renderer::setConf(const GraphicsConf &conf) {
...
int GL20Renderer::getWidth() const {
	return width;
}

int GL20Renderer::getHeight() const {
	return height;
}

float GL20Renderer::getScalingFactor() const {
	return (float) drawWidth / width;
}

void GL20Renderer::tick() {
