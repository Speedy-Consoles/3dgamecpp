...
/*
	...
	as of 2015-03-22
	The original name of the file was acutil_unicode.cpp

	2015-03-22 - Lars Thorben Drögemüller
		Removed enclosing namespace and enforced naming convention
*/
...
int decodeUTF16(const char *encodedBuffer, unsigned int *outLength, UnicodeByteOrder byteOrder)
{
	const unsigned char *buf = (const unsigned char *)encodedBuffer;
	int value = 0;
	if (byteOrder == UC_LITTLE_ENDIAN) {
		value += buf[0];
...
int encodeUTF16(unsigned int value, char *outEncodedBuffer, unsigned int *outCharLength, UnicodeByteOrder byteOrder)
{
	if (value < 0x10000) {
		if (byteOrder == UC_LITTLE_ENDIAN) {
